...
class Beallitasok:
    def __init__(self):
        ...
        self.fps = 120  # A kliens cél képkockaszáma.
        ...
        self.jatekos_sebesseg = 10.0  # A pattogós játékos alap mozgási sebessége.
        ...
        self.patogo_min_sebesseg = 3.0  # A pattogó ellenség minimális komponens-sebessége.
        self.patogo_max_sebesseg = 4.0  # A pattogó ellenség maximális komponens-sebessége.
...
class VilagAllapot:
    ...
    def frissites(self) -> None:
        if self.jatek_mode == "alma":
            self._alma_mod_frissites()
        else:
            self._patogos_mod_frissites()
    ...
    def _alma_mod_frissites(self) -> None:
        ...
        for kigyo in self.osszes_kigyo():
            ...
            if kigyo.jatekos_e:
                ...
                uj_dx, uj_dy = self._legjobb_irany_ai(kigyo)
    ...
    def _kigyo_fej_leptetes(self, kigyo: KigyoAdat) -> None:
        ...
        kigyo.test_pontok[0][1] = fej_y + kigyo.irany_y * kigyo.sebesseg
    ...
    def _kozelebbi_almak(self, x: float, y: float) -> List[Tuple[float, float]]:
        eredmeny = []
        for kulcs in KorSeged.szomszed_kulcsok(self.beallitasok.racsok_nagysaga, x, y, self.beallitasok.racsok_nagysaga * 2, 2):
    ...
    def _legjobb_irany_ai(self, kigyo: KigyoAdat) -> Tuple[float, float]:
        ...
        alap_szog = math.atan2(kigyo.irany_y, kigyo.irany_x)
        legjobb_dx = kigyo.irany_x
        legjobb_dy = kigyo.irany_y
        legjobb_pont = float("-inf")
        lato_tav = kigyo.sebesseg * self.beallitasok.kigyo_lato_ido

        for fok in self.beallitasok.kigyo_dontesi_szogek:
            szog = alap_szog + math.radians(fok)
            dx = math.cos(szog)
            dy = math.sin(szog)
            becsult_x = fej_x + dx * lato_tav
            becsult_y = fej_y + dy * lato_tav
            pont = 100000.0 - KorSeged.tavolsag(becsult_x, becsult_y, cel_x, cel_y)

            fal_puffer = kigyo.sugar * self.beallitasok.kigyo_veszely_puffer
            if becsult_x < fal_puffer or becsult_x > self.beallitasok.vilag_szelesseg - fal_puffer:
                pont -= 1_000_000
            if becsult_y < fal_puffer or becsult_y > self.beallitasok.vilag_magassag - fal_puffer:
                pont -= 1_000_000

            for minta in (0.35, 0.65, 1.0):
                px = fej_x + dx * lato_tav * minta
                py = fej_y + dy * lato_tav * minta
                pont -= self._minta_veszely_pont(kigyo, px, py)

            if pont > legjobb_pont:
                legjobb_pont = pont
                legjobb_dx = dx
                legjobb_dy = dy
    ...
    def _celpont_kereses(self, kigyo: KigyoAdat, almak: List[Tuple[float, float]]) -> Tuple[float, float]:
        ...
        if legjobb is None:
            return float(self.veletlen.randint(0, self.beallitasok.vilag_szelesseg)), float(self.veletlen.randint(0, self.beallitasok.vilag_magassag))
        return legjobb
    ...
    def _patogos_mod_frissites(self) -> None:
        if not self.jatekosok and not self.patog_ellenseg:
            return

        self._mozgas_pattogos_jatekosok()
        self._mozgas_pattogo_ellensegek()
        self._pattogok_potlas(max(0, self.max_patogok - len(self.patog_ellenseg)))
        self._pattogok_racsozasa()

    def _mozgas_pattogos_jatekosok(self) -> None:
        for jatekos in self.jatekosok.values():
            ...
            ndx, ndy = KorSeged.normalizal(dx, dy)
            uj_x = jatekos.x + ndx * jatekos.sebesseg
            uj_y = jatekos.y + ndy * jatekos.sebesseg
    ...
    def _mozgas_pattogo_ellensegek(self) -> None:
        for pattogo in self.patog_ellenseg:
            pattogo.x += pattogo.dx
            pattogo.y += pattogo.dy
